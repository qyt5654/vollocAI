...
| 向量存储 | 余弦相似度内存引擎 + Milvus 双引擎 | Milvus 1536维 IVF_FLAT，内存兜底 |
...
POST /ai/chat
    ...
    └─ AiTaskExecutor (线程池异步)
         ...
         ├─ 会话历史加载 (Redis)
         └─ 路由执行:
              ├─ 文本对话 → UnifiedAgent.SIMPLE (流式+工具)
              └─ 根因分析 → UnifiedAgent.DEEP (计划→执行→报告)
                   │
         ┌────────┴────────┐
         ▼                 ▼
...
### 4.7 UnifiedAgentService（统一 Agent）

融合了原 ReActAgent + SupervisorAgent，一个类实现两种模式：

- **SIMPLE 模式**：流式输出 + 工具调用。模型通过 JSON 协议自主判断是否需要工具，不需要时直接流式回答。每个 token 实时推送前端。
- **DEEP 模式**：Supervisor 制定计划 → Executor (AgentToolPlanner) 分步执行工具 → Replanner 评估 → 生成结构化报告。
- **路由**：关键词含"根因/报告/排查"走 DEEP，其余走 SIMPLE

- **知识点**: Agent 模式统一、流式+工具共存、多步编排
- **关键代码**: `UnifiedAgentService.java`
...
| 模式 | 引擎 | 状态 |
|------|------|------|
| 生产 | Milvus — 1536维 IVF_FLAT + IP 度量 | ✅ Docker 部署，已联调通过 |
| 降级 | `VectorStore` — 余弦相似度 + 优先队列 Top-K | 零依赖，Milvus 不可用时自动切换 |

**流程**：文档分片(≤500字自动整存) → DashScope Embedding(1536维) → Milvus 写入 → 向量检索 Top-3 → 注入 Prompt → LLM 生成。

**切换方式**：`DocumentService` 通过 `@Autowired(required=false) MilvusServiceClient` 检测，有 Milvus 走 Milvus，无则走 VectorStore。`milvus.enabled=true/false` 控制 `MilvusConfig` 是否加载。

- **知识点**: RAG 架构、Embedding、余弦相似度、Milvus 向量数据库、IVF_FLAT 索引、分片策略
...
- *工作职责*：\
  ...
  *3.*实现统一 Agent 服务：SIMPLE 模式（流式+工具自主调用）和 DEEP 模式（Supervisor→Planner→Executor→Replanner 多步编排），根据问题复杂度自动路由。\
  *4.*实现 RAG 检索增强生成：Milvus 向量库(1536维 IVF_FLAT) + 自研余弦相似度内存引擎双模，文档分片→DashScope Embedding→检索→注入Prompt；上层接口统一，Milvus 不可用时自动降级。\
...
| 类别 | 文件 | 说明 |
|------|------|------|
| **Agent** | `UnifiedAgentService.java` | 统一 Agent（SIMPLE流式 / DEEP多步） |
| | `AgentToolPlannerService.java` | 工具规划执行 |
| **AI 服务** | `MultimodalAIServiceImpl.java` | 动态模型调用(streamChat/generateImage/Voice) |
| | `IntentRecognitionService.java` | 意图三分类（text/image/voice） |
| | `AiTaskExecutor.java` | 线程池异步执行器 |
